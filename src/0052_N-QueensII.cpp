...
public:
    ...
    int backtrack1(int size, vector<string> &board, int row, set<int> &cols, set<int> &diagonals, set<int> &antidiagonals){
        if(row == size)
            return 1;
        int total = 0;
        for(int col = 0; col < size; col++){
            ...
            total += backtrack1(size, board, row + 1, cols, diagonals, antidiagonals);
    ...
    int totalNQueens1(int n) {
        vector<string> board(n, string(n, '.'));
        set<int> cols, diagonals, antidiagonals;
        return backtrack1(n, board, 0, cols, diagonals, antidiagonals);
    }

    int totalNQueens(int n) {
        //return solveNQueens1(n).size();
...
int main() {
    ...
    for(auto test: vecTests){
        ...
        auto output = sol.totalNQueens(test.input);
